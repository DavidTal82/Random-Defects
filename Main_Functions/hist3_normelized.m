function [nn,ctrs] = hist3_normelized(varargin)
...
if strcmp(varargin{cellfun(@ischar,varargin)},'Normalization') 
    ind = find(cellfun(@ischar,varargin));
    norm_value = varargin{ind+1};
    temp = cell(1,ind-1);
...
if isempty(x)
    ...
    % Deviding n by the number of elments to normelize
    % David Jan 22/2015 
    if ~isempty(norm_value)
    n = n/max(max(n))*norm_value;
    end
end
